...
class ExpenseSummaryStats(APIView):
    ...
    def get(self, request):
        ...
        expenses = Expense.objects.filter(owner=request.user, date__gte=year_ago, date__lte=today_date)
        final = {}
...
class IncomeSourcesSummaryStats(APIView):
    ...
    def get(self, request):
        today_date = datetime.date.today()
        year_ago = today_date - datetime.timedelta(days=30 * 12)
        income = Income.objects.filter(owner=request.user, date__gte=year_ago, date__lte=today_date)
        final = {}
